...
```bash
git clone https://github.com/tibor19/TechWorkshop-L300-GitHub-Copilot-and-platform.git
...
1. Provision all Azure resources (ACR, App Service, Application Insights, Azure OpenAI)
...
3. Explore the deployed resources:
   ...
   - **Azure OpenAI**: Manage AI model deployments
...
### Issue: "Quota exceeded for Azure OpenAI"

**Solution**: 
1. Request quota increase in Azure Portal
2. Or disable OpenAI deployment by editing `infra/main.parameters.dev.json`:
   ```json
   "enableOpenAI": {
     "value": false
   }
...
- Azure OpenAI (S0): Pay per token usage
